...
import { Search, Plus, User, Check, Edit2, X, List, Clock, Trash2 } from 'lucide-react';
...
export function MobilePage() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f23]">
      {/* Header fixo */}
      <header className="sticky top-0 z-50 bg-[#0f0f23]/95 backdrop-blur-sm border-b border-[#009b3a]/30 px-4 py-3">
        <div className="flex items-center justify-between mb-3">
          <img src="/cantai_logo.png" alt="CantAI" className="h-10" />

          {/* Botão da fila */}
          <button
            onClick={() => setShowQueue(!showQueue)}
            className="relative flex items-center gap-2 bg-[#009b3a] hover:bg-[#00762c] text-white px-3 py-2 rounded-full text-sm font-medium transition-colors"
          >
        ...
        {isEditingName ? (
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                autoFocus
                className="w-full bg-[#1a1a2e] text-white pl-10 pr-4 py-2 rounded-lg border border-[#009b3a]/50 focus:border-[#009b3a] focus:outline-none"
              />
            </div>
            <button
          ...
          <button
            ...
            className="flex items-center gap-2 w-full bg-[#1a1a2e]/50 text-white px-4 py-2 rounded-lg border border-[#009b3a]/30 hover:border-[#009b3a]/50 transition-colors"
          >
            <User className="w-4 h-4 text-[#009b3a]" />
      ...
      {showQueue && (
        <div className="fixed inset-0 z-40 bg-black/80 backdrop-blur-sm" onClick={() => setShowQueue(false)}>
          <div
            className="absolute top-0 left-0 right-0 max-h-[70vh] bg-[#0f0f23] border-b border-[#009b3a]/30 overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="sticky top-0 bg-[#0f0f23] px-4 py-3 border-b border-[#374151] flex items-center justify-between">
              <h2 className="text-white font-bold flex items-center gap-2">
                <List className="w-5 h-5 text-[#009b3a]" />
            ...
            {queue.length === 0 ? (
              <div className="p-8 text-center text-gray-400">
                <img src="/cantai_logo.png" alt="CantAI" className="h-12 mx-auto mb-3 opacity-50" />
              ...
              <div className="p-4 space-y-3">
                {queue.map((item, index) => (
                  ...
                  >
                    <span className="text-[#009b3a] font-bold text-lg w-6">{index + 1}</span>
                    <img
                      src={item.thumbnail}
                      alt={item.songTitle}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[#009b3a] text-xs flex items-center gap-1">
      ...
      {/* Busca */}
      <div className="sticky top-[108px] z-30 bg-[#0f0f23]/95 backdrop-blur-sm px-4 py-3 border-b border-[#374151]">
        <form onSubmit={handleSearch} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-[#1a1a2e] text-white pl-10 pr-4 py-3 rounded-lg border border-[#374151] focus:border-[#009b3a] focus:outline-none"
            />
          </div>
          <button
            type="submit"
            disabled={isSearching}
            className="bg-[#009b3a] hover:bg-[#00762c] text-white px-4 py-3 rounded-lg font-medium transition-colors disabled:opacity-50"
          >
            {isSearching ? '...' : 'Buscar'}
          </button>
        </form>
      ...
      <main className="p-4">
        {isLoading ? (
          <div className="flex items-center justify-center py-12">
            <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-[#009b3a]"></div>
          </div>
        ) : videos.length === 0 ? (
          <div className="text-center py-12 text-gray-400">
            <img src="/cantai_logo.png" alt="CantAI" className="h-16 mx-auto mb-4 opacity-50" />
          ...
          <div className="space-y-3">
            {videos.map((video) => {
              ...
              return (
                <div
                  key={video.code}
                  className="flex items-center gap-3 bg-[#1a1a2e]/50 rounded-xl p-3 border border-[#374151]/50"
                >
                  {/* Thumbnail */}
                  <div className="relative">
                    ...
                    />
                    <span className="absolute -top-1 -left-1 bg-[#009b3a] text-white text-xs px-1.5 py-0.5 rounded font-mono">
                      #{video.code}
                    </span>
                  </div>
                  ...
                  <button
                    ...
                    className={`flex items-center justify-center w-12 h-12 rounded-full transition-all ${
                      inQueue
                        ...
                        : isAdding
                        ? 'bg-[#009b3a]/50 text-white animate-pulse'
                        : 'bg-[#009b3a] hover:bg-[#00762c] text-white active:scale-95'
                    }`}
                  >
                    {inQueue ? (
      ...
      <footer className="fixed bottom-0 left-0 right-0 bg-gradient-to-t from-[#0f0f23] to-transparent py-6 px-4 pointer-events-none">
